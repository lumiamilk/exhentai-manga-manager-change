...
### v1.0.15 (2026-03-22) - 多版本评论合并与缓存功能 [已发布]
- **[功能] 多版本评论合并**
  - 目标：进入漫画详情页时，自动搜索同名漫画的所有版本，按语言优先级加载评论
  - 优先级：中文 > 英文 > 日文 > 韩文 > 其他
  - 找到中文评论后停止获取其他语言评论
  - 架构：
    ```
...
- **[功能] 评论缓存**
  - 一周内查看同一漫画：直接从数据库加载评论，无网络请求
  - 一周后查看：重新从网络获取评论，更新缓存
  - 缓存存储：SQLite 数据库 `CommentCache` 表

- **已完成工作**:
  1. ✅ 数据库模型 - `modules/database.js` 添加 `CommentCache` 模型
  2. ✅ IPC 接口 - `index.js` 添加 `get-comment-cache`、`save-comment-cache` handlers
  3. ✅ 表同步 - 启动时 `CommentCache.sync()` 创建表
  4. ✅ 前端逻辑 - `BookDetailDialog.vue` 实现缓存检查和多版本搜索
  5. ✅ 语言检测 - 从标题识别 `[chinese]`、`[english]` 等语言标签
  6. ✅ 语言优先级 - 中文 > 英文 > 日文 > 韩文 > 未知
  7. ✅ 评论去重 - 按内容前100字符去重
  8. ✅ 评论来源标签 - 红色"汉化"、蓝色"英文"
  9. ✅ 序列化修复 - `foundLink` 转为 `foundLinkHrefs` 避免克隆错误

- **涉及文件**:
  - `modules/database.js` - 添加 `prepareCommentCacheModel`
  - `index.js` - 添加缓存 IPC handlers 和表同步
  - `src/components/BookDetailDialog.vue` - 多版本评论加载逻辑
