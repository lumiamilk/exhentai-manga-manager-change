...
@routes.post("/cancel-all")
async def cancel_all_tasks(request):
    """清空所有等待中的任务队列"""
    global QUEUE
    ...
    for task_id in list(QUEUE):
        ...
        if task_id in TASK_STATES:
            ...
            TASK_STATES[task_id]['info'] = 'cancelled'
    # 清空队列
    QUEUE.clear()
    print(f'Cancelled {cancelled_count} queued tasks')
    return web.json_response({'status': 'success', 'cancelled': cancelled_count})
